...
from ..models import Visitante, Sendero, RegistroVisita, Encuesta, Usuario, Comentario
import logging
import gc
...
def generar_reporte_completo():
    ...
    try:
        ...
        visitas = RegistroVisita.objects.select_related("visitante").order_by(
            "-fecha_visita"
        )[:MAX_ROWS_PER_SHEET]
        ...
        try:
            ...
            stats_pais = (
                Visitante.objects.values("nacionalidad")
                .annotate(total=Count("id"))
                .order_by("-total")[:50]
            )  # Limitar a top 50 países
            ...
            try:
                ws_usuarios.append(
                    [
                        ...
                        f"{usuario.nombre or ''} {usuario.apellido or ''}".strip()[:40]
                        or "N/A",
                        (
                            usuario.get_rol_display()[:20]
                            if hasattr(usuario, "get_rol_display")
                            else "N/A"
                        ),
                    ]
                )
        ...
        try:
            ...
            encuestas = (
                Encuesta.objects.select_related("visita")
                .only("id", "formulario", "visita__id")
            ...
            ws_encuestas.append(
                ["Error al cargar encuestas", str(e)[:50], "N/A"]
                + ["N/A"] * len(claves_formulario)
            )
        ...
        # Cerrar conexiones de DB si es necesario (de forma segura)
        try:
            connection.close()
        except Exception:
